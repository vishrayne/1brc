...
public class SegmentKeyInterner {
    ...
    private static final ThreadLocal<SimpleAggregatorMap<SegmentKey, SegmentKey>> THREAD_LOCAL_CACHE = ThreadLocal.withInitial(
            () -> new SimpleAggregatorMap<>(512)); // Modest initial capacity for thread-local map
    ...
    private static final int THREAD_LOCAL_CACHE_EVICTION_THRESHOLD = 1024; // Tune as needed

    public static SegmentKey sharedIntern(MemorySegment stationNameSlice) {
    ...
    public static SegmentKey threadLocalIntern(MemorySegment stationNameSlice) {
        ...
        SimpleAggregatorMap<SegmentKey, SegmentKey> localCache = THREAD_LOCAL_CACHE.get();
        ...
        return localCache.computeIfAbsent(tempKeyForLookup, k -> k);
    }
     ...
     */
    public static SegmentKey threadLocalAndSharedIntern(MemorySegment stationNameSlice) {
